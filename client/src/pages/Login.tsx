...
export function Login() {
  ...
  async function submit(e: Event) {
    ...
    setError('');
    setLoading(true);
    const { data } = await api<{ token: string; tenant: { status: string } }>('POST', '/auth/login', { email, password });
    setLoading(false);
    if (!data.success || !data.data) { setError(data.error?.message || 'Login failed'); return; }
      ...
      <div class="w-full max-w-md">
        ...
        <form onSubmit={submit} class="bg-white rounded-3xl shadow-elevated p-8">
          ...
          <button
            type="submit"
            disabled={loading}
            class="w-full mt-6 py-3 bg-brand-400 hover:bg-brand-500 text-gray-900 font-semibold rounded-full transition-all hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50"
          >
...
function Field({ label, type, value, onInput, placeholder }: any) {
  return (
    <div>
      ...
      <input
        ...
        class="w-full px-4 py-3 bg-surface-100 rounded-xl text-sm border-0 focus:ring-2 focus:ring-brand-400 focus:bg-white transition-all"
      />
    </div>
  );
}
