...
interface PipelineStats { total: number; by_status: Record<string, number>; }
interface PipelineJob { id: string; status: string; current_stage: string; created_at: string; }
interface FulfillmentJob { id: string; waybill: string; milestone: string; }
interface Status { status: string; ecommerce_platform?: string; shopify_plan?: string; courier?: string; }

export function Overview() {
  const [, navigate] = useLocation();
  const [loading, setLoading] = useState(true);
  const [status, setStatus] = useState<Status>({ status: '-' });
  const [stats, setStats] = useState<PipelineStats>({ total: 0, by_status: {} });
  const [jobs, setJobs] = useState<PipelineJob[]>([]);
  const [fulfillment, setFulfillment] = useState<FulfillmentJob[]>([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (s.data.success) setStatus(s.data.data);
      if (st.data.success) setStats(st.data.data);
      if (j.data.success) setJobs(j.data.data.jobs);
      if (f.data.success) setFulfillment(f.data.data.jobs);
      setLoading(false);
    })();
  }, []);
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2">
          <Card
            ...
            action={
              <button onClick={() => navigate('/pipeline')} class="text-xs text-brand-600 font-semibold hover:underline">
                View all
              </button>
            }
          >
            {jobs.length === 0 ? (
              <EmptyState title="No activity yet" description="Send an order email to trigger the pipeline." />
            ) : (
              <div class="space-y-3">
                {jobs.map((j) => (
                  <div key={j.id} class="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                    <div>
        ...
        <div class="space-y-6">
          ...
          <div class="bg-dark-800 rounded-3xl p-6 text-white">
            ...
            {agentList.map((name) => (
              <div key={name} class="flex items-center gap-3 py-1.5">
          ...
          <Card
            title="Fulfillment"
            action={
              <button onClick={() => navigate('/fulfillment')} class="text-xs text-brand-600 font-semibold hover:underline">
                View
              </button>
            }
          >
            {fulfillment.length === 0 ? (
              <p class="text-sm text-gray-400">No active jobs</p>
            ) : (
              fulfillment.slice(0, 4).map((j) => (
                <div key={j.id} class="flex items-center justify-between py-1.5">
...
function milestoneStatus(m: string): string {
  if (m === 'delivered') return 'completed';
  if (m === 'cancelled' || m === 'failed') return 'failed';
  return 'processing';
}
