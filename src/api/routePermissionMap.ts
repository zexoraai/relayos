...
export const ROUTE_PERMISSION_MAP: RouteSpec[] = [
  ...
  },
  {
    router: 'packerAuthRoutes',
    method: 'POST',
    path: '/packer-auth/orders/:id/revert',
    permission: 'auth-only',
    justification: 'gated by packerAuthMiddleware — packer reverts their own mistaken status flips on assigned orders only',
  },
  {
    router: 'packerAuthRoutes',
    method: 'GET',
    path: '/packer-auth/queue',
    permission: 'auth-only',
    justification: 'gated by packerAuthMiddleware — packer reads only their own assigned packing queue',
  },
  {
    router: 'packerAuthRoutes',
    method: 'GET',
    path: '/packer-auth/manual-upload-queue',
    permission: 'auth-only',
    justification: 'gated by packerAuthMiddleware — packer reads only their own assigned manual-upload queue',
  },
  {
    router: 'packerAuthRoutes',
    method: 'POST',
    path: '/packer-auth/manual-upload-queue/:id/complete',
    permission: 'auth-only',
    justification: 'gated by packerAuthMiddleware — packer submits waybill+pin only for orders assigned to themselves',
  },
  {
    router: 'packerAuthRoutes',
    method: 'GET',
    path: '/packer-auth/collection-queue',
    permission: 'auth-only',
    justification: 'gated by packerAuthMiddleware — packer reads only their own assigned collection queue',
  },
  {
    router: 'packerAuthRoutes',
    method: 'POST',
    path: '/packer-auth/collection-queue/:id/confirm',
    permission: 'auth-only',
    justification: 'gated by packerAuthMiddleware — packer confirms collection only for orders assigned to themselves',
  },
  {
    router: 'packerAuthRoutes',
    method: 'GET',
